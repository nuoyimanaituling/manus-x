...
from app.domain.services.prompts.system import get_skill_prompt
...
class PlanActFlow(BaseFlow):
    def __init__(
        ...
        skill_prompt = get_skill_prompt(skill_loader.get_descriptions())
        ...
        # Add skill tool when skills are available
        ...
        # Add scheduled task tool when service is provided
        if scheduled_task_service and user_id:
            tools.append(ScheduledTaskTool(scheduled_task_service, user_id))
        ...
        # Inject skill prompt into agent system prompts
        if skill_prompt:
            self.planner.system_prompt = self.planner.system_prompt + skill_prompt
            self.executor.system_prompt = self.executor.system_prompt + skill_prompt
            logger.info(f"Injected skill prompt into agents for Agent {self._agent_id}")
