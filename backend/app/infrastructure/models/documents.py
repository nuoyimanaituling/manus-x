...
class ScheduledTaskDocument(BaseDocument[ScheduledTask], id_field="task_id", domain_model_class=ScheduledTask):
    """MongoDB document for ScheduledTask"""
    task_id: str
    user_id: str
    name: str
    ...
    class Settings:
        ...
        indexes = [
            "task_id",
            "user_id",
            IndexModel([("status", ASCENDING), ("next_run_at", ASCENDING)]),
        ]


class ScheduledTaskExecutionDocument(BaseDocument[ScheduledTaskExecution], id_field="execution_id", domain_model_class=ScheduledTaskExecution):
    """MongoDB document for ScheduledTaskExecution"""
    execution_id: str
    task_id: str
    user_id: str
    ...
    class Settings:
        ...
        indexes = [
            ...
            "task_id",
            "user_id",
            IndexModel([("task_id", ASCENDING), ("scheduled_at", ASCENDING)]),
        ]