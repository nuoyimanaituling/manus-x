        ...
        <div
            ...
            <footer class="flex flex-row justify-between w-full px-3">
                <div class="flex gap-2 pr-2 items-center">
                    ...
                    <!-- Scheduled Task Mode Button -->
                    <button @click="toggleScheduledMode"
                        :class="[
                            'rounded-full border inline-flex items-center justify-center gap-1 clickable cursor-pointer text-xs px-3 h-8 shrink-0 transition-colors',
                            isScheduledMode
                                ? 'bg-white text-[#0066FF] border-[#0066FF]'
                                : 'border-[var(--border-main)] text-[var(--text-secondary)] hover:bg-[var(--fill-tsp-gray-main)]'
                        ]">
                        <Calendar :size="16" />
                        <span class="hidden sm:inline">{{ t('Scheduled Task') }}</span>
                    </button>
                </div>
...
const placeholderText = computed(() => {
    return isScheduledMode.value
        ? t("Describe scheduled task, e.g. 'Send me market news every morning at 8'")
        : t('Give Manus a task to work on...');
});

const emit = defineEmits<{
    (e: 'update:modelValue', value: string): void;
    (e: 'submit', scheduledMode: boolean): void;
    (e: 'stop'): void;
}>();

const toggleScheduledMode = () => {
    isScheduledMode.value = !isScheduledMode.value;
};
...
const handleSubmit = () => {
    ...
    emit('submit', isScheduledMode.value);
    // Reset scheduled mode after submit
    if (isScheduledMode.value) {
